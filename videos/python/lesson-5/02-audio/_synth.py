"""
Per-scene Edge TTS synthesis + concat + loudnorm for CodeMong Python Lesson 5.

Lesson 5 ("조건문 — if / elif / else / 중첩", 14 scenes, target ~213s).

Inherits the lesson-3 baseline pipeline (inline "(N초 정적)" silence handling).
This script is identical in logic to videos/python/lesson-3/02-audio/_synth.py;
only paths and the docstring lesson reference change. Lesson 5 has TWO scenes
with active-recall silence beats — scene-06 ("(1.5초 정적)"), scene-10 and
scene-13 ("(1.8초 정적)") — and the regex+split flow handles them all uniformly.

Pronunciation dict 2nd-pass result (this round): 0 new entries needed.
The script-writer pre-substituted every English/numeric token in the
voiceover_to_record block. Verified by grep over the block:
  - if / elif / else / else if → "이프" / "엘리프" / "엘스" / "엘스 이프"
  - True / False               → "트루" / "폴스"
  - score / admin              → "스코어" / "어드민"
  - 60 / 75 / 45 / 40          → "육십" / "칠십오" / "사십오" / "사십"
Variable identifiers (`is_login`, `is_admin`) never appear in narration —
the script-writer paraphrases them as "로그인 여부" / "어드민 여부", which is
the desired UX for first-encounter Python identifiers. `print` is only on
screen, never spoken (carried over from lesson-3 scene-10 mental model).
...
# ---------- paths ----------
ROOT = Path(__file__).resolve().parents[4]  # …/codemong  (02-audio → lesson-5 → python → videos → codemong)
