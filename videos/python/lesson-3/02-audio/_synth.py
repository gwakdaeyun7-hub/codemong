"""
Per-scene Edge TTS synthesis + concat + loudnorm for CodeMong Python Lesson 3.

Lesson 3 ("변수와 자료형 — 변수 / 숫자·문자열·불린 / print()", 13 scenes, target ~212s).

Inherits the lesson-2 baseline pipeline (inline "(N초 정적)" silence handling).
This script is identical in logic to videos/python/lesson-2/02-audio/_synth.py;
only paths and the docstring lesson reference change. The inline-silence
regex + split/synth/concat flow is the new baseline for any lesson with an
active-recall beat (lesson-3 scene-07: "(1.5초 정적)").

Pronunciation dict 2nd-pass result (this round): 0 new entries needed.
The script-writer pre-substituted all English/numeric tokens in the
voiceover_to_record block ("프린트 괄호", "트루", "오", "육"). Variable names
(name / age / is_student) are intentionally left as English — Edge TTS reads
them with their standard romanized Korean pronunciation, which is the desired
learner experience for first-encounter Python identifiers.
...
# ---------- paths ----------
ROOT = Path(__file__).resolve().parents[4]  # …/codemong  (02-audio → lesson-3 → python → videos → codemong)
