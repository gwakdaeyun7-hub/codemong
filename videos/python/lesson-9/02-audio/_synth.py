"""
Per-scene Edge TTS synthesis + concat + loudnorm for CodeMong Python Lesson 9.

Lesson 9 ("함수 — def / 매개변수 / return / 지역·전역 변수", 11 scenes, target 180s).

Inherits the lesson-8 baseline pipeline (inline "(N초 정적)" silence handling).
This script is logically identical to videos/python/lesson-{5,6,7,8}/02-audio/_synth.py;
only paths and the docstring lesson reference change. Lesson 9 has TWO scenes
with active-recall silence beats — scene-06 ("(1.5초 정적)") and scene-09
("(1.5초 정적)") — and the regex+split flow handles 0/1/2/3+ markers per scene
without modification.

Pronunciation dict 2nd-pass result (this round): 0 new entries needed.
Script-writer already pre-substituted every English token in the
voiceover_to_record block:
  - `def` → "데프", `return` → "리턴", `print` → "프린트", `None` → "논"
    (all 4 already in dict; script-writer added `def`/`return`/`else` during
    lesson-9 prep, bumping dict from 60 → 63 entries; current size 66 after
    earlier lesson-8 additions remain).
2nd-pass grep over voiceover_to_record block confirmed only short identifier
tokens remain (`a`, `b`, `x`), which the lesson-3+ identifier-pronunciation
policy keeps as TTS-natural English-letter readings ("에이", "비", "엑스").
No dict entries needed for those.

Identifier pronunciation policy (lesson-3+): variables that *are* spoken get a
dict entry; variables that are visual-only do not. Lesson 9 *speaks* single-letter
identifiers `a`, `b`, `x`, `f` and short English identifiers `add`, `double`,
`name`, `greet`, `result` — all are short enough that TTS-natural English
reading is acceptable; no dict entries needed.
...
# ---------- paths ----------
ROOT = Path(__file__).resolve().parents[4]  # …/codemong  (02-audio → lesson-9 → python → videos → codemong)
