"""
Per-scene Edge TTS synthesis + concat + loudnorm for CodeMong Python Lesson 7.

Lesson 7 ("리스트 — 생성 / 인덱싱 / 추가·삭제 / 2차원 기초", 13 scenes, target 180s).

Inherits the lesson-6 baseline pipeline (inline "(N초 정적)" silence handling).
This script is identical in logic to videos/python/lesson-6/02-audio/_synth.py
plus ONE new behavior: when a scene contains "(N초 정적)" markers, the resulting
timestamps.json entry records sub-clip durations so downstream composer can
verify R-004 (Active Recall reveal sync). Lesson 7 has ONE active recall beat —
scene-08 ("(2초 정적)" before the answer reveal).

Pronunciation dict 2nd-pass result (this round): 0 new entries needed.
Dictionary at 48 entries (script-writer added `append`/`del`/`grid`/`in`/`len`/
`list` in the previous round). 2nd-pass grep over voiceover_to_record confirmed
script-writer pre-substituted every English token:
  - for / in                       → 포 / 인
  - append / del / len             → 어펜드 / 델 / 렌
  - range / print                  → 레인지 / 프린트
  - list / grid                    → 리스트 / 그리드
  - scores / s                     → 스코어즈 / 에스 (variable names, not dict)
Loop variable `s` and `i` are intentionally never spoken as English chars —
narration uses 한국어 "에스" / "이" (carried over from lesson-3 `is_student`
policy: variables that *are* spoken get a dict entry, variables that are
visual-only or spoken in Korean phonetic form do not).
...
Produces:
  ...
  timestamps.json          — [{ sceneId, startMs, endMs, narrationText, subClips? }]
                             aligned to final mp3; subClips populated for scenes
                             with inline silence markers (a0, s1, a2, ...).
...
Pipeline:
  ...
  5. Build timestamps.json with cumulative boundaries rescaled to match the final mp3 duration
     ...
     is added so the composer can verify R-004 reveal sync.
"""
...
# ---------- paths ----------
ROOT = Path(__file__).resolve().parents[4]  # …/codemong  (02-audio → lesson-7 → python → videos → codemong)
...
async def main() -> None:
    ...
    # composer can sync R-004 reveal frames against actual audio offsets.
