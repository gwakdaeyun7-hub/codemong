"""
Per-scene Edge TTS synthesis + concat + loudnorm for CodeMong Python Lesson 8.

Lesson 8 ("딕셔너리 & 자료구조 — dict / tuple / set 기초", 11 scenes, target 180s).

Inherits the lesson-6 baseline pipeline (inline "(N초 정적)" silence handling).
This script is logically identical to videos/python/lesson-{5,6,7}/02-audio/_synth.py;
only paths and the docstring lesson reference change. Lesson 8 has ONE scene
with an active-recall silence beat — scene-05 ("(1.5초 정적)") — and the
...
voiceover_to_record block (`dict`/`tuple`/`set`/`get`/`KeyError`/`None`/`key`/
`index`/`value`/`Python` → 딕셔너리/튜플/셋/겟/키 에러/논/키/인덱스/밸류/파이썬).
2nd-pass grep over voiceover_to_record block confirmed zero English tokens
in narration bodies. Pronunciation dictionary now at 66 entries (up from
lesson-6's 42 — lesson-8 adds 9 new entries for dict/tuple/set/get/key/value/
KeyError/None/index, plus prior +15 from lesson-7 prep).

Identifier pronunciation policy (lesson-3+): variables that *are* spoken get a
dict entry; variables that are visual-only do not. Lesson 8 *speaks* `scores`
as 한국어 "스코어스" (자연 한국어 발음 — no dict entry), `좌표` and `수강과목`
are already 한국어 (no dict needed), `students` is visual-only (no dict needed).
...
# ---------- paths ----------
ROOT = Path(__file__).resolve().parents[4]  # …/codemong  (02-audio → lesson-8 → python → videos → codemong)
