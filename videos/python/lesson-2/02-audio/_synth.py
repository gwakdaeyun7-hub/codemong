"""
Per-scene Edge TTS synthesis + concat + loudnorm for CodeMong Python Lesson 2.

Lesson 2 ("코딩의 표현 방법", 13 scenes, target ~211s).

Differences from lesson-1 baseline:
  - scene-06 narration contains an inline "(1.5초 정적)" stage direction that must be
    rendered as a real 1500ms silent segment between the two narration halves
    (active-recall beat — learner needs the gap to predict the answer).
    The lesson-1 baseline simply stripped such markers; here we split the narration
    on the marker, synth each half independently, and join them with a silent clip.
...
Pipeline:
  ...
  5. Build timestamps.json with cumulative boundaries rescaled to match the final mp3 duration
     ...
     so downstream caption/wire pipelines never see a stage direction; the actual silence
...
# ---------- paths ----------
ROOT = Path(__file__).resolve().parents[4]  # …/codemong  (02-audio → lesson-2 → python → videos → codemong)
