 ...
 *
 * Stage 3 — composition wired with audio + timestamps.json-derived timing.
 *
 * Each scene is wrapped in a <Sequence> with timing from `timing.ts`.
 * The scene-local frame inside each Scene component starts at 0 because
 * <Sequence> auto-shifts useCurrentFrame() relative to its `from` prop.
 *
...
export const Lesson4: React.FC = () => {
  return (
    <AbsoluteFill style={{ background: "#fafafa" }}>
      ...
      <Audio src={staticFile("python-lesson-4/voiceover.mp3")} />
    </AbsoluteFill>
  );
};
