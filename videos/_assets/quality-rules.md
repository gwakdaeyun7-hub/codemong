...
- **Stage 3 wire 필수 — placeholder reveal 잔존 금지**: Active Recall scene 의 정답 reveal `delaySec`(`answerSwap` / `swapA` / `swapB` 등)이 Stage 2 placeholder(목표 길이 기준) 값으로 남으면, 실측 audio 가 placeholder 보다 길어진 만큼 정답이 질문/정적 구간에 떠버려 R-004 를 위반한다. wire 단계(`--from=04-render` 직전)에서 `_scenes/sNN.{a0,s1,a2}.mp3` 를 ffprobe 로 실측해 위 범위로 **반드시 re-sync** 할 것. 일반 scene 의 tail dead-air 와 달리 Active Recall reveal 은 narration 에 의미적으로 묶여 있어 rescale 금지 주석만으로는 부족하다. (재발: 2026-05-23, lesson-9 scene-06/09 — placeholder 잔존으로 production 재발.)
