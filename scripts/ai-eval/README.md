...
## weekly-comment.mjs — 주간 성장 리포트 LLM 코멘트 시나리오 점검 (2026-09-21)

채점 루브릭과 달리 점수를 결정하지 않는 프롬프트(`lib/skill/weekly-comment-prompt.ts`)라
회귀 스위트 대신 **대표 숫자 시나리오 8개를 돌려 사람이 읽어 보는** 용도. 프로덕션 프롬프트 모듈을
그대로 import 하므로 드리프트 없음. 읽기 전용(DB 쓰기 없음).

```bash
pnpm ai:weekly            # 프로젝트 루트에서. 8 시나리오 출력
pnpm ai:weekly --save     # weekly-comment-results.json 에 저장 (프롬프트 수정 전후 비교)
pnpm ai:weekly --only=A,D # 일부만
```

볼 것: "~해요"체 유지 / 통과율 낮을 때 사실대로 말하는지 / 전주 대비 변화가 있으면 가장 많이 오른 축을
잘한 점으로 쓰는지 / 약한 축별 행동 지침이 맞게 나오는지 / 전 축 동점(A)일 때 약점을 지어내지 않는지 /
"완벽·인상적·과제·부문" 같은 금지어가 없는지.
