...
- 특정 입력값에만 맞춘 하드코딩(입력과 무관한 고정 출력, 공개 예시 값만 분기 처리)은 일반적인 풀이가 아니다. 이 경우 개념 이해도와 문제 해석력 점수는 각각 40점을 넘을 수 없다. 감점 폭을 키워서라도(필요하면 한 축에 60점 이상 감점) 두 축 모두 40 이하가 되게 하라.
