...
· 실행 중 빨간 오류가 나면 마지막 줄의 오류 이름과 줄 번호부터 읽어 보세요. (12강)
· 채점은 무작위를 고정해서 확인하니, 위 순서대로 문제 은행을 만드세요.
...
· 9강에서는 함수에 숫자·문자열만 넘겼지만, 여기서는 주문 리스트를 함수에 그대로 넘겨 담고 읽습니다.
...
· 반복은 while True: 와 break 로, 메뉴 확인은 if 이름 in 메뉴: 로 합니다.
