...
export const pythonLesson12Content: LessonContent = {
  ...
  video: {
    ...
    transcriptSummary:
      "이 영상에서는 새로운 문법을 배우는 대신, 내가 짠 코드가 제대로 작동하지 않을 때 어떻게 해결해야 하는지 알아봅니다. 빨간 에러 메시지(Traceback)의 마지막 줄부터 읽으며 문법 오류를 찾아내는 방법, 에러는 없지만 결과가 틀릴 때 print() 문으로 중간값을 확인하며 논리 오류를 잡는 방법을 배웁니다. 그래도 해결하기 어렵다면 해당 개념 강의로 돌아가거나, AI에게 코드와 에러 메시지를 함께 질문하는 방법까지 살펴봅니다.",
