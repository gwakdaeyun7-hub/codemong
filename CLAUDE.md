...
- **Prisma 7.8** → Supabase Postgres (singleton: `lib/prisma.ts`, generated client: `lib/generated/prisma/`). 모델 15개: Post / Comment / PostLike / CommentLike / LessonLike / PostReport / CommentReport / LessonProgress / ProjectProgress / ExerciseProgress / ExerciseAttempt / Notification / ProblemSubmission / WeeklyReport / ExamSchedule
...
| `/courses/[courseId]/lessons/[lessonId]` | 강의 상세 (개념 탭) | **로그인 필수** (영상 게이팅 — 페이지 레벨 `redirect('/login?next=...')`, proxy 아님). `lesson-1` ~ `lesson-12` (파이썬 개요·개발환경 / 코딩의 표현 방법 / 변수와 자료형 / 입력과 연산자 / 조건문 / 반복문 / 리스트 / 딕셔너리 & 자료구조 / 함수 / 모듈 & 랜덤 / 파일 입출력 / 디버깅 & AI 활용) 매칭. `lesson-1`~`lesson-12` 는 **영상-only 모드** — 강의 헤더 + 영상 카드 + LessonLikeBar(좋아요+댓글 카운트) + CommentSection(댓글 임베드) + 이전/다음 네비. 우측 통계·진행률은 `LessonProgress` 실데이터(`getCourseLessonStatuses`) — 단 강 완료 판정은 이제 영상 학습완료뿐 아니라 그 강 연습 전부 통과까지 **AND**(연습 0개 강은 면제). **`lesson-13`~`lesson-15` 는 프로젝트형**(13 계산기·14 랜덤 복습 퀴즈·15 키오스크) — 영상 대신 `ProjectRunner`(코딩테스트 스타일: 텍스트 문제 1개를 읽고 단일 에디터에 코드를 한 번에 완성 → 종합 테스트케이스로 Pyodide 채점, 통과 시 `passProjectAction`). 로그인 게이팅·헤더 동일(`LessonContentHeader subtabs=[]` 재사용). 프로젝트 강의 간 이전/다음 네비 활성 — 번호±1 로 잡고 영상이든 프로젝트든 등록돼 있으면 링크(15강만 next 없음) |
| `/courses/[courseId]/lessons/[lessonId]/practice` | 강별 연습 문제 | **로그인 필수**(통과 기록 저장 — 비로그인은 `redirect('/login?next=...')`). `getExercises` 있는 강만(현재 1·3~12강). `ExerciseRunner`(문제 칩 자유 전환 + CodeMirror + 실행(입력 모달)/제출 채점 — 13강 ProjectRunner 단순화, `lib/project/grader.ts` 재사용). **통과 배너 = 다음 행동 안내**: 선택한 문제가 통과 상태면(재방문 포함) 배너가 뜨고, 남은 문제가 있으면 [다음 문제]→다음 **미해결** 문제, 이 강 연습 **전부 통과**(순번 아닌 통과 개수 판정)면 "이 강 연습을 모두 통과했어요." + [강의로 돌아가기](`lessonHref` prop). 상단의 작은 "강의로 돌아가기" 링크는 중간 이탈용으로 그대로 둠(앱 공통 패턴 — 키우지 말 것). 진입점 2곳: 강의 목록 카드 "연습" 버튼 + 영상 상세 `PracticeEntryLink` 카드. **통과 저장 구현(`ExerciseProgress`, 문제별 ✓ — 연습 페이지 칩(`initialPassed` 복원)/강의 목록·영상 상세 카드 "N/M 통과"). 이수율 연동 완료 — 강 완료 = 영상 학습완료 AND 그 강 연습 전부 통과(연습 0개 강은 면제). `getCourseLessonStatuses`/`getCourseCompletion` 에 반영** |
...
| `/mypage/calendar` | 학습 캘린더 | **실데이터 연결 완료** — 연속 학습일 배너 + 월별 캘린더(**색상=그날 대표 문법 테마 / 진하기=학습 깊이**(연함=영상만·시도만, 짙음=문제·연습 통과) / 우상단 체크 배지=오답 복습 해결) + 날짜 클릭 시 학습 리포트(수강 강의, 문제 수·정답률, 오답 태그). 집계는 `getLearningCalendar` (기존 4개 모델만 사용). + **시험 D-Day 카드**(여러 개 등록/삭제, 가장 가까운 시험 크게, **D-14 이내면 약점 단원 보강 추천 알림**, 시험일은 캘린더 칸에 rose 링·점으로 표시). + **배지 5종도 실데이터 파생**(`deriveBadges` — 저장 모델 없이 볼 때 계산, 획득 조건을 칸에 명시, N/M 획득 표시) |
| `/mypage/weekly` | 주간 성장 리포트 대시보드 | **로그인 필수**(`redirect('/login?next=/mypage/weekly')`). 지난주(KST 월~일) 1주만 다룸. 위에서부터 ① 히어로 = `WeeklyReportCard variant="full"`(미니 레이더 지난주 vs 전주 스냅샷 + 축별 점수·전주 대비 변화 + 약한 축 안내 + LLM 코멘트 — 축 점수·delta·코멘트는 여전히 `WeeklyReport` 행 "열 때 생성") ② 요일별 활동 막대 차트(강의/연습/문제) + KPI 4타일 ③ 지난주에 푼 실력향상 문제 목록 ④ 자주 난 오류·AI 감점 축 태그. ②~④ 집계는 `getWeeklyDashboard`(읽기 전용, 새 저장 모델 없음). 섹션 컴포넌트는 `components/mypage/weekly-dashboard.tsx` |
...
| `lib/course-detail.ts` | `CourseDetail` 타입 + `pythonCourseDetail` + `getCourseDetail(id)`. `stats.totalHours` = **"약 6시간"**(lesson-plan durationMinutes 합 375분 기준 = 영상 12강 265 + 프로젝트 13~15강 40/50/60 — 실력향상 34문제는 별도 트랙이라 제외), `enrolledCount`/`rating`/`reviews` 는 실데이터가 없어 **미노출**(조건부 렌더 — 가짜 수치·후기 금지 원칙). `roadmap` 6단계는 **실제 구현된 기능 기준**으로 적는다(1 개념설명=강의 영상 / 2 개념응용=강별 연습 문제 / 3 문제해결=실력향상 문제 자동 채점 / 4 학습완료=영상+연습 완료 시 기록 / 5 성장피드백=AI 진단·5축 레이더·주간 리포트). 아직 없는 단계(6 다음단계추천)는 **"준비 중"이라고 그대로 적는다** — 없는 기능을 "이렇게 진행돼요"로 쓰지 말 것(패턴 결정 11 가짜 수치 금지와 같은 원칙). `cta.subtext` 는 실제 분량("영상 12강 + 프로젝트 3강, 예상 6시간")을 적고, 버튼이 할 일을 문장으로 다시 설명하지 않는다 |
| `lib/lesson-plan.ts` | `Lesson`(정적 메타만 — `kind?: "video" \| "project"`, **`status` 없음**) / `LessonWithStatus`(= `Lesson` + 사용자별 status, 렌더용) / `LessonStatus` / `LessonBadge`(표시용 + `hint?`) 타입 + `pythonLessonPlan` (15강, lesson-13~15=project) + `getLessonPlan(id)`. `totalLessons` 15. **뱃지 배열 없음** — 획득 여부는 `stats-queries.ts` 의 `deriveBadges` 가 파생. **`durationMinutes` = 영상 ffprobe 실측(분 반올림) + 그 강 연습 문제 수 × 7분** (합 375분 = 6시간 15분 — 영상 12강 265 + 프로젝트 13~15강 40/50/60. 영상 실측 합계는 43분 — 강당 2.6~4.5분). 영상 재렌더·연습 추가 시 이 값도 갱신, UI 라벨은 "예상 N분" |
...
| `lib/project-content.ts` | 프로젝트형 강의 콘텐츠. `Project` / `ProjectExample` / `TestCase` / `ExpectedOutput` 타입 + `pythonLesson13Project` / `pythonLesson14Project` / `pythonLesson15Project` + `projectByRef` / `getProject(courseId, lessonId)` (python/be-python 둘 다 매칭). **코딩테스트(백준/프로그래머스) 스타일** — 스텝 누적이 아니라 문제 1개를 한 번에 완성해 제출. `Project` = `prompt`(전체 요구사항) + `examples`(입출력 예시) + `starterCode` + `solutionCode`(완전한 모범답안) + `hints`(힌트 사다리) + `tests`(종합 `TestCase[]`) + `concepts`(필요 개념 태그) + `seed?`(채점 시 random 고정 — 10강 `Exercise.seed` 와 같은 규약, "실행"은 무작위·"제출"만 시드). 채점은 전부 stdin→stdout 시나리오. 14강 「랜덤 복습 퀴즈」(seed 2026, 딕셔너리 리스트 은행 8문항 + `randint`/`used` 리스트로 중복 없이 출제, 함수 `ask`/`save_result`, `result.txt` 쓰고 읽어 출력, 테스트 8 = 순서 무관 7 + `[seed]` 1, 지문 첫 줄은 "7~12강에서 배운 것을 중심으로" — 문법 범위 자체는 1~12강) / 15강 「키오스크」(메뉴 딕셔너리 4개, `while True` 주문 반복·"종료", 없는 메뉴·수량<1 처리, `receipt.txt` 저장 후 읽어 출력, 함수 `show_menu`/`add_order`/`make_receipt`, 테스트 8). 둘 다 1~12강 영상 대본 범위 문법만 — `strip/lower/split`·`and/or/not`·`random.sample/shuffle`·`try/except`·f-string·`for key in dict:`(딕셔너리 순회) 미사용(8강 대본에 딕셔너리 순회가 없어 15강 `show_menu` 는 이름 리스트 `names` 를 따로 두고 순회 + `menu[name]` 조회로 대체 — 7강 리스트 순회 + 8강 조회 조합). 예외로 지문에서 소개한 뒤 쓰는 것 둘: `"\n"`(15강 [규칙])·함수에 리스트를 넘기는 패턴(9강 영상은 숫자·문자열 인자만 — 15강 [규칙]에서 예고) |
| `lib/exercise-content.ts` | `Exercise` / `ExerciseSet` 타입 + `pythonLesson4Exercises` (4강 「입력과 연산자」 3문제) + `exercisesByRef` / `getExercises(courseId, lessonId)`. 13강 `project-content.ts` 와 **별개**인 강별 연습 문제 트랙 — 영상 보고 그 강 문법만으로 푸는 단일 채점 미니 문제(강당 0~3, `note` = 영상 밖 보충 고정글). 채점은 `lib/project/grader.ts` 재사용, `tests` 는 13강 `TestCase` 공유. **1·3~12강(2강=코드없음 제외) 11개 강 26문제 + 러너 UI(`components/exercise/`)·진입점(강의 목록/상세)·`/practice` 라우트 구현. 10강 random 은 `Exercise.seed` 로 채점만 결정적 고정(실행은 무작위). 통과 저장 구현(`ExerciseProgress`)·이수율 연동 완료(강 완료 = 영상 학습완료 AND 그 강 연습 전부 통과, 연습 0개 강은 면제)** |
| `lib/project/grader.ts` | **클라이언트 전용** 코드 실행+채점 엔진. Pyodide(CDN 동적 로드) 로 `exec`, `input()` 모킹 + `print` stdout 캡처. `matchesExpected` 는 `ExpectedOutput` 순차 소비(숫자 허용오차 1e-9 / 텍스트 부분일치). export: `preloadPyodide` / `runPythonProgram` / `runPythonInteractive`(대화형 실행 — input() 마다 화면 내 입력 모달, 채점 아닌 '직접 돌려보기' 용) / `matchesExpected` / `gradeStep` + 타입 `CaseResult` / `RunResult`. `gradeStep`/`runPythonProgram` 의 `seed?` 옵션은 wrapper 에 `random.seed(N)` 을 주입해 random 을 결정적으로 채점(10강 연습·14강 프로젝트용). **실행마다 `tempfile.mkdtemp()` 로 chdir**(채점·대화형 래퍼 모두) — 파일 입출력(11강) 잔재를 실행 간 격리, 끝나면 원래 cwd 복귀. **대화형 실행 래퍼(`buildAsyncWrapper`)는 학습자 코드의 모든 `def` → `async def`, 그 함수 호출 → `await 호출` 로 텍스트 변환** — 함수 안에서 `input()` 을 불러도 동작(14·15강 "함수로 기능 분리"). 입문자 코드(최상위 def + 단순 호출) 범위만 — 람다·메서드·고차 함수 미지원. 문자열 리터럴 안의 `input(` 도 치환되므로 지문·은행 문장에 `input()` 을 글자로 넣지 말 것 |
...
| `lib/learning/exercise-actions.ts` | Server Actions: `passExerciseAction(lessonRef, exerciseId)` — 로그인 필수, 한 문제 통과를 `ExerciseProgress.passed` 맵에 멱등 병합(이미 통과면 write 생략). 통과가 이수율을 바꾸므로 `revalidatePath` 가 강의 목록·practice 에 더해 **영상 상세(`/courses/[courseId]/lessons/[lessonId]`)·홈(`/`)·마이페이지(`/mypage`)** 까지 확장(강좌 소개 `/courses/[courseId]` 는 이수율 미사용이라 제외). 채점은 클라(Pyodide), 통과 결과만 기록. 13강 ProjectProgress 패턴 답습. / `recordAttemptAction(lessonRef, exerciseId, summary)`(+`AttemptSummary` = hadError/errorType/casesPassed/casesTotal) — 제출마다 `ExerciseAttempt` 에 append(통과·오답 모두), 로그인 필수·존재하는 강/문제만·수치 방어적 정규화. 성장 레이더 실데이터용 로그라 `revalidatePath` 안 함(이수율 반영·레이더 무효화는 `passExerciseAction` 이 함께 처리 → 중복 회피) |
...
| `lib/learning/lesson-themes.ts` | 학습 캘린더 문법 테마 — `LESSON_THEMES` 4그룹(basics 1~4강 초록 / control 5·6강 보라 / data 7·8강 파랑 / advanced 9~15강 주황) + `themeOfLesson`/`getTheme`. Tailwind 클래스는 동적 생성 불가라 lightClass·deepClass·dotClass 를 정적 문자열로 보유. prisma 미import — 서버·클라 공용 |
| `lib/learning/calendar-queries.ts` | 학습 캘린더 집계 — `getLearningCalendar(userId, monthsBack=6)` → `{days, streak, activeDays}`. 날짜 키는 **KST**(`kstDateKey`). 소스 4개(LessonProgress 시청/완료 · ProblemSubmission · ExerciseAttempt · ProjectProgress)를 날짜 버킷에 모아 `depth`(통과 있으면 deep) / `themeKey`(활동 최다 강의의 테마, 동점이면 진도 앞선 강) / `themes`(그날 전체) / `revisitSolved`(**이전에 틀린 문제를 그날 처음 통과** — 문제·연습 각각) / 영상 목록 / 제출·통과 수 / `tags`(단원명+파이썬 예외명+AI 감점 축, 빈도순 3개)를 계산. streak 은 오늘 활동이 없으면 어제부터 셈(오늘은 미완료로 간주) |
| `lib/learning/exam-queries.ts` | 시험 D-Day 조회 + 보강 추천 — `kstTodayKey`/`dateKeyToUtcDate`/`daysBetween`(날짜 키 유틸) + `listUpcomingExams`(오늘 이후, 가까운 순, dday 포함) + `getWeakestLesson`(**푼 단원 중 점수 최저 → 없으면 미학습 앞 단원**, 사용자 확정 규칙. 점수는 AI 3축 평균, 없으면 해결률. 문제당 최신 제출 1건·skipped_empty 제외로 레이더와 집계 규칙 일치) |
| `lib/learning/exam-actions.ts` | Server Action: `createExamAction(title, date)` / `deleteExamAction(id)`. 제목 40자·날짜 형식·과거 날짜 금지·2년 이내·사용자당 10개 상한 검증, 삭제는 `deleteMany` 로 소유자 조건 동봉. revalidate: /mypage/calendar |
| `lib/learning/skill-radar.ts` | 성장 레이더(스파이더) 어댑터 — **D단계 실측 승급 완료**. `PYTHON_SKILL_AXES`(코드 품질 5축) + `getSkillRadarDetail(courseId, userId)` → `{points, meta}` (meta=`SkillRadarMeta`: deterministicLive/aiLive/averageLive/**hasUserData** — 카피·렌더 조건부용) + `getSkillRadar`(기존 시그니처 wrapper) + `getLessonSkillRadar(courseId, lessonId, userId)`(단원별 — 사용자 표본 0이면 null). **결정적 2축 = ExerciseAttempt(연습)+ProblemSubmission(시험) 합산 실집계, AI 3축 = ProblemSubmission 의 Gemini 점수 실집계**. **집계 규칙(사용자 확정): 문제/연습당 최신 제출 1건만** (반복 제출로 평균 부풀리기 차단, AI 축은 문제당 최신 ok 1건, 난이도 가중 미적용). 코호트 평균 = 전 유저 실집계 `unstable_cache` 5분(표본 유저<2 또는 샘플<5 이면 **평균 곡선을 숨김**). **데모값 없음(`DEMO_VALUES` 삭제) — 표시되는 숫자는 전부 실측**: 표본 없는 축은 0 + `RadarPoint.userMeasured=false`(카피가 "측정 전" 표기), `meta.hasUserData=false` 면 호출부가 차트 대신 안내를 렌더 |
| `lib/learning/stats-queries.ts` | 마이페이지 학습 현황·최근 학습·뱃지의 실데이터 소스. `LearningStats` 타입 + `getLearningStats(courseId, userId)`(완료 강의/전체 · 해결 문제/전체 · 연습 통과/전체 · streak · activeDays · commentCount) + `RecentLessonItem` + `listRecentLessons(courseId, userId, limit=3)`(LessonProgress+ProjectProgress `updatedAt` 최신순, 완료 여부 포함) + `deriveBadges(stats)`(뱃지 5종 획득 여부 파생 — 첫걸음=강의1개완료 / 연속 학습=3일 / 연습 벌레=연습5개통과 / 문제 해결=문제1개해결 / 완주=전강완료. 저장 모델 없이 볼 때 계산). **새 집계 규칙을 만들지 않고 기존 단일 진실 원천(`getCourseCompletion`/`getLearningCalendar`/`getCourseSolveStatuses`/`getCourseExerciseStatuses`)만 조합** — 그래서 홈·강의 목록·캘린더와 숫자가 어긋나지 않는다 |
| `lib/problems/` | **실력향상(시험) 트랙 문제 은행** — 연습 트랙(`exercise-content.ts`)과 별개. `types.ts`(`Problem`: publicTests/hiddenTests 분리 + solutionCode + rubricNote / `ProblemSet` / `ClientProblem`=solutionCode 제외 클라 전송용) + `lesson-{4..9}.ts` **34문제**(4·5·6·7강 각 6, 8·9강 각 5 — 억지 단원 금지 원칙으로 1·3·10·11·12강 제외, 지문·데이터 전부 자체 제작, 평문 지문(줄표·불릿 금지), starterCode="# 아래에 코드를 작성하세요" 통일(9강만 def 뼈대 예외)) + `index.ts`(`listProblemSets`/`getProblemSet`/`getProblem`/`toClientProblem`, python/be-python 둘 다 매칭). 채점 규약(`TestCase`)은 13강 것 공유. 히든 케이스는 클라 채점 특성상 번들 포함 — UI 미노출이 방어의 전부(기존 정책과 일관) |
| `lib/skill/submission-actions.ts` | Server Action: `submitProblemAction(lessonRef, problemId, code, summary)` — 로그인 필수, 방어적 정규화(코드 10KB·케이스 수 클램프), **2단계 write**: ① `ProblemSubmission` append-only 저장(제출 기록은 AI 와 무관하게 항상 확정) ② AI 게이트(키 존재→하루 한도) 통과 시 Gemini 3축 채점 후 같은 행 update(ok/failed — best-effort, 실패해도 제출 불변). 반환값에 `ai: SubmissionAiPayload` 포함 — 클라가 즉시 AI 진단 카드를 그림. lessonRef 는 정식 courseId 기준 "be-python/lesson-N". revalidate: 문제·단원·/skill·/mypage |
| `lib/skill/submission-queries.ts` | `listMySubmissions`(문제 페이지 내 제출, 최신순) / `getCourseSolveStatuses(courseId, userId)` → `{byLesson: {solved,total}, solvedProblems}` (passed=true distinct 집계, 존재하는 문제만 카운트). 비로그인이면 빈 결과 |
| `lib/skill/weekly-report-queries.ts` | 주간 리포트 순수 로직 — `kstWeekStartDate`(KST 월요일, @db.Date PK 규약)/`kstWeekInstantRange`/`buildWeeklyReportData`(주간 제출 수·통과율 + 현재 레이더 스냅샷 + 전주 리포트 대비 delta + weakestAxis)/`toPayload`. **축 스냅샷은 실제 측정된 축만 기록**(미측정 축을 0점으로 넣으면 그 축이 항상 최저가 되어 `weakestAxis` 추천이 왜곡됨). **측정된 축이 전부 같은 점수면 `weakestAxis = null`**(첫 축이 억지 약점으로 잡히던 문제 수정). Cron 승급 시 build 재사용 seam |
| `lib/skill/weekly-report-actions.ts` | Server Action: `getOrCreateWeeklyReportAction()` — **"열 때 생성"**(cron/Batch 없음 — Batch 는 무료 티어 불가 확인). 지난주 리포트 있으면 반환, 없고 지난주 제출≥1 이면 생성(복합 PK 멱등, 충돌 시 재조회), 제출 0이면 행 없이 null. `llmComment` 는 Gemini best-effort(숫자 요약만 입력, 실패 시 null — 숫자 리포트는 유지). 프롬프트는 `weekly-comment-prompt.ts` SSOT — 인라인 프롬프트 없음 |
| `lib/skill/weekly-comment-prompt.ts` | **주간 리포트 LLM 코멘트 프롬프트 SSOT**(순수 모듈 — prisma·서버 import 없음, 클라 번들 가능). export: `WEEKLY_COMMENT_AXES`(축 key/label/meaning/action 5개) / `WEEKLY_COMMENT_SYSTEM` / `WEEKLY_COMMENT_SCHEMA` / `buildWeeklyCommentInput(data)` / `WeeklyCommentInput`. 규칙: "~해요"체 고정, 낮은 통과율도 사실대로, 전주 대비 변화가 있으면 가장 많이 오른 축을 잘한 점으로, 약한 축별 행동 지침 1개, 금지어(완벽·인상적·과제·부문), 비교 대상·약점이 없으면 입력에 "없음"을 명시해 지어내기 차단. `weekly-report-actions.ts`·`weekly-report-card.tsx` 가 함께 import. 수정 시 `pnpm ai:weekly` 로 눈검사(점수화 없음 — 채점 루브릭과 달리 점수를 결정하지 않는 저위험 프롬프트라 회귀 스위트 대신) |
| `lib/skill/weekly-dashboard-queries.ts` | `/mypage/weekly` 집계 — `getWeeklyDashboard(userId, weekStartDate)`(React `cache()`, 읽기 전용, 새 저장 모델 없음). 지난주(KST, `kstWeekInstantRange`) 범위의 `LessonProgress`(videoWatchedAt/learnCompletedAt)·`ExerciseAttempt`·`ProblemSubmission` 을 읽어 `WeeklyDashboard`{weekStart, weekEnd, days[7](videos/exercises/submissions/active), activeDays, videos(distinct lessonRef), exercisesPassed(casesPassed>=casesTotal distinct), exerciseAttempts, submissionCount, solvedProblems, passRate, problems[], tags[], hasActivity}. 존재하지 않는 문제는 목록에서 제외(해결 현황 규칙과 동일). 축 점수·delta·LLM 코멘트는 여전히 `WeeklyReport` 행이 담당 |
| `lib/ai/config.ts` | `isAiEnabled()`(GEMINI_API_KEY 유무) / `getGeminiModel()`(기본 **gemini-3.1-flash-lite** — 원래 계획한 2.5 Flash-Lite 는 2026-07 기준 신규 사용자 404 라 후속 저가 티어로 대체, env 교체 seam) / `getAiDailyLimit()`(기본 10). 키 없으면 AI 기능 전부 조용히 off. 서버 전용 — NEXT_PUBLIC 금지. 참고: 2026-07 신규 발급 Gemini 키는 `AIza...` 가 아니라 `AQ.` 로 시작하는 새 형식 |
| `lib/ai/gemini.ts` | Gemini REST fetch 래퍼(SDK 없음). 구조화 출력(responseMimeType+responseSchema)·temperature 0.2·`thinkingBudget: 0`(출력 한도 잠식 방지)·AbortController 15s·`GeminiError` 정규화. `AI_DEBUG=1` env 면 프롬프트/응답 전문을 서버 콘솔에 출력(프롬프트 튜닝용) |
| `lib/ai/rubric.ts` | **AI 3축 채점 프롬프트 SSOT** — `RUBRIC_SYSTEM_PROMPT`(고정 루브릭: 개념/효율/해석 감점 방식 + **힌트형 지시(정답 코드 절대 미제시)** + 인젝션 방어 + 캘리브레이션 규칙(하드코딩 40 이하·출력 형식 코드 직접 확인·미완성 개념 상한·실행오류 의도 기준·실패 시 3축 100 불가)) / `buildProblemBlock`/`buildSubmissionBlock`(캐싱 최적화 순서: 고정→문제별 고정→가변 — 순서 바꾸면 implicit caching 깨짐) / `RUBRIC_RESPONSE_SCHEMA` / `validateAiVerdict`(수제 validator·clamp 0~100). 루브릭 기준은 docs/발표-근거-리서치.md §2 표. **수정 시 `scripts/ai-eval/` 회귀 스위트 필수** (README 참조 — 규칙 하나가 다른 시나리오를 회귀시킨 전례 있음. 2026-07-16 검증: 재현성 10/10, 34문제 전수 정상, 전문가 대조 ±20 이내 93~96%, 인젝션 10종 방어. 2026-07-28 변별력 전수 매트릭스: 34문제 × V1~V4 품질 사다리 136채점 **34/34 전 기준 통과** — `scripts/ai-eval/discrimination/`) |
| `lib/ai/limits.ts` | 하루 AI 한도 — `kstStartOfToday`(KST 유틸, 서버 타임존 비의존) / `countTodayAiCalls`(ProblemSubmission `[userId,createdAt]` count, skipped_* 미차감) / `hasAiQuotaToday` |
| `lib/ai/grade-submission.ts` | `gradeSubmissionWithAi(problem, code, summary)` → `AiVerdict` — rubric+gemini 조합 헬퍼. 실패는 GeminiError throw (호출부가 aiStatus=failed 처리) |
...
| `components/lessons/` | 강의 목록 페이지 (course-progress-header — 3지표 중 완료 N/M·진행률은 실데이터, "총 학습 시간"은 커리큘럼 기준값(사용자 누적 시간 아님 — 라벨 바꾸지 말 것), lesson-list, lesson-card, progress-stat-card, stats-card, tips-card, badges-card — **`deriveBadges` 실데이터 + N/M 획득 표시**, 미획득은 자물쇠 + 획득 조건 툴팁) |
| `components/lesson-content/` | 강의 상세 영상 영역 (lesson-content-header, lesson-video-card — **Client Component**(영상 90% 시청 추적 + 학습 완료 버튼, `captionsSrc` 있으면 `<track kind="subtitles" srcLang="ko">` 렌더 — default 미지정이라 기본 꺼짐·브라우저 CC 버튼으로 토글), lesson-navigation — 영상-only 모드) |
| `components/project/` | 프로젝트형 강의 실행 UI (project-runner — **Client Component**(코딩테스트 스타일 단일 문제 러너: 문제·입출력 예시·단일 에디터·실행(대화형 입력 모달)/제출·콘솔·케이스별 채점결과·힌트 사다리. 스텝 칩/진행바/해금 없음 — 통과 시 `passProjectAction`. `seed` 있는 프로젝트는 제출 채점만 시드 고정 — `gradeStep(code, project.tests, project.seed)`), code-editor — CodeMirror 6 래퍼, ProjectRunner 에서 `dynamic(ssr:false)` 로 로드(브라우저 전용)) |
...
| `components/skill/` | 실력향상 시험 환경 (problem-runner — **Client**(프로그래머스식 2-pane: 좌 지문 카드/우 다크 에디터 존(solution.py 탭·콘솔·초기화/코드 실행/제출 버튼 바), 히든 마스킹("히든 N" ✓/✗), 제출 시 `submitProblemAction`+`router.refresh()`, AI 진단은 "loading"→payload 로 즉시 표시, 대화형 입력 없음 — 준비된 테스트 데이터로만 실행·채점), ai-feedback-card(3축 점수·감점 근거·힌트형 피드백 — skipped_limit/failed 는 안내 카피, skipped_no_key 는 미렌더), submission-history — **Client**(내 제출 펼침: 코드 원문+AI 진단(aiStatus=ok 시)), difficulty.ts(하/중/상 라벨·칩 톤), weekly-report-card — **Client**(mount 시 `getOrCreateWeeklyReportAction` 호출 "열 때 생성". `variant?: "summary" | "full"` — summary(기본, `/skill`·`/mypage`)=눈썹 "주간 성장 리포트" + "지난주 M월 D일 ~ M월 D일"(weekStart 로 계산) + 제출·통과율 타일 + LLM 코멘트 + 카드 폭 보라 버튼 [리포트 자세히 보기 →](`/mypage/weekly`) / full(`/mypage/weekly` 히어로)=미니 레이더(`SkillRadarChart` 재사용 — userValue=지난주 스냅샷, averageValue=전주 스냅샷(=지난주−delta), 전주 리포트 없으면 두 번째 폴리곤 미렌더) + 축별 점수 바·전주 대비 변화(약한 축 강조) + 약한 축 안내 + LLM 코멘트. 축 라벨은 `lib/skill/weekly-comment-prompt.ts` 의 `WEEKLY_COMMENT_AXES` 에서 import — skill-radar 는 prisma import 라 클라 번들 불가), icon-map.ts(named re-export 화이트리스트)) |
...
| `components/mypage/` | 마이페이지 (사이드바(mypage-sidebar — NAV 에 "주간 성장 리포트" `/mypage/weekly` 항목, 학습 캘린더 다음, 아이콘 `mypageIcons.chart`=ChartColumnBig), 프로필 카드, **학습 현황 카드(mastery-stats-card — `LearningStats` prop 으로 실측 4지표. 퀴즈 미구현인 "평균 이해도"는 지표에서 제외)**, **최근 학습 카드(recent-activity-card — `RecentLessonItem[]` prop)**, 성장 레이더 차트(growth-report-card — 코드 품질 5축·나 vs 전체 평균, **표본 0이면 차트 대신 안내 카드 + /skill CTA**, 미측정 축은 "측정 전" 카피, 평균 범례는 `averageLive` 일 때만; skill-radar-chart — **순수 SVG, 의존성 0, N축 동적**, 코호트 표본 부족 시 평균 곡선 미렌더, `ariaLabel?` prop(기본값 기존 문구 — 주간 리포트 full 카드가 "전주 vs 지난주" 설명으로 교체)), weekly-dashboard — **Server**(`/mypage/weekly` 섹션 3개: `WeeklyActivitySection` 요일별 활동 그룹 막대 차트(순수 SVG, 강의/연습/문제 3계열, y축 정수 눈금·hairline 격자, 0 활동도 회색 자리표시, 막대 위 값 라벨, `<title>` 툴팁, 범례) + KPI 4타일(학습한 날 N일/7일 · 본 강의 N개(영상 90% 이상) · 연습 통과 N개(시도 N회) · 문제 제출 N회(통과율·해결 N문제)) / `WeeklyProblemsSection` 지난주에 푼 문제 목록(단원·번호·제목·난이도 칩·정답/오답·케이스 best/total·시도 횟수·마지막 errorType, 문제 페이지 링크, "N/M 해결") / `WeeklyTagsSection` 자주 난 오류(errorType, rose)·AI 감점 축(amber) 빈도순 최대 8개. 데이터는 `getWeeklyDashboard` prop), learning-calendar — **Client**(학습 캘린더: 연속일 배너·월 이동·테마 색/깊이 칸·오답 복습 체크 배지·날짜별 리포트. 집계는 서버에서 끝내고 렌더·선택만 담당, 오늘 날짜(KST)도 서버가 prop 으로 전달), exam-dday-card — **Client**(시험 D-Day: 등록/삭제 폼·가까운 시험 D-Day·D-14 이내 보강 추천 알림. mutation 알림은 useToast, 폼 검증은 inline), 닉네임/비번 변경 폼, settings-section wrapper) |
...
| `ProjectProgress` | project_progress | 프로젝트형 강의(13강~) 진도. lessonRef+userId 복합 PK. `stepStatuses`(스텝별 통과 여부) / `submittedCode`(스텝별 작성 코드, 이어하기) 둘 다 `Json @default("{}")`, `completedAt`=채점 스텝 전부 통과 시점(=학습완료, 이수율 집계). 단일 문제 코딩테스트인 13~15강은 `"solution"` 키 하나만 사용(같은 ProjectRunner·passProjectAction 경로) |
...
| `ExamSchedule` | exam_schedules | 시험 일정 — 학습 캘린더 D-Day/보강 추천용. `userId`+`title`(40자)+`examDate`(@db.Date, KST 날짜를 UTC 자정으로 — WeeklyReport.weekStart 규약 동일). 인덱스 `[userId, examDate]`. 지난 시험도 행은 남기되 화면에는 다가오는 것만 노출, 사용자당 10개 상한 |
...
- 마이페이지 = 내 댓글 / 내 글 / 좋아요한 강의/글 / 학습 캘린더(실데이터)
...
8. **카피 톤**: 한국어, 입문자 친화, 정직. "쉬워요!" 같은 과장 금지. 이모지 거의 안 씀. **UI 가 이미 보여주는 것을 말로 다시 설명하지 않는다** — "버튼을 누르면 ~됩니다"·"여기서 확인할 수 있어요" 같은 조작 재설명, 같은 화면 위아래에서 같은 안내를 두 번 하는 것(예: `/practice` 페이지 헤더와 `ExerciseRunner` 상단) 금지. **동기부여·감탄·느낌표를 넣지 않는다** — "열공 중!"·"축하합니다!"·"균형 있게 성장할 수 있어요" 류. 완료·통과 문구는 사실만 적는다(`exercise-runner` 통과 배너가 기준). **내부 개발 용어를 화면에 쓰지 않는다** — "백엔드 라운드에서 추가될 예정" 대신 "아직 지원하지 않아요". **인증 화면 포함 전 화면이 "~해요"체** — 하십시오체·과공손체("입력하시면 …보내드립니다") 금지. 새 카피는 쓰기 전에 이 기준으로 자기 점검할 것 (2026-09-23 전수 정리에서 프로젝트 지문·연습 note·학습 캘린더·시험 D-Day·마이페이지 부제·인증 화면이 이 기준으로 수정됨).
...
10. **프로젝트형 강의(13~15강)**: 영상 없이 텍스트 문제 → 직접 코드 작성·실행·채점하는 **코딩테스트 스타일**(문제 1개를 한 번에 완성해 제출, 스텝 누적 아님) 미션 (영상 강의 1~12강과 별개 트랙). 실행 엔진은 **Pyodide**(브라우저 Python, CDN 로드 — 서버 채점 X). 13강 「계산기 만들기」는 **1~6강 문법만** 사용 (함수·리스트·딕셔너리·try·split 미사용) — 강의가 다룬 범위 안에서만 풀리도록. 14강 「랜덤 복습 퀴즈」(7~12강 도구 중심의 중간 종합, seed 채점)·15강 「키오스크」(1~12강 전체 흐름 설계의 최종 종합, 무작위 없음 — 무작위는 14강 몫)도 같은 패턴으로 구현됨. **문법 범위는 14·15강 모두 1~12강이고(7~12강 도구는 1~6강 기초 없이 못 씀), 차이는 무엇을 연습시키느냐에 있다** — 13 기초 제어 / 14 도구 / 15 흐름 설계. 새 프로젝트를 만들 때 지문·은행 문장에 `input()` 을 글자로 쓰지 말 것(대화형 실행 래퍼가 치환), 그리고 영상 대본 grep 으로 범위를 확인하고 범위 밖 표현이 꼭 필요하면 지문 [규칙]에서 소개한 뒤 쓸 것.
...
pnpm ai:weekly        # 주간 리포트 LLM 코멘트 시나리오 8개 눈검사 (프롬프트 수정 시, --save 로 전후 비교, --only=A,D)
...
py scripts/generate_captions.py     # 강의 영상 자막(WebVTT) 생성 — 인자 없으면 12강 전체, 강 번호 지정 가능
...
- **Claude Code 도구 세션에서 `pnpm dev`(Turbopack) 가 전 페이지 500**: 비대화형 자동화 환경에서 Next 16 기본 Turbopack 으로 띄우면 "Ready" 까지는 뜨지만, 첫 요청에서 `app/globals.css` PostCSS 처리용 node child 프로세스 spawn 이 `0xc0000142` 로 죽어 전 페이지 500. 사용자 본인 터미널·Vercel 에서는 정상 — 코드 문제가 아니므로 설정을 바꾸지 말 것. 회피: `pnpm exec next dev --webpack` 으로 띄우면 PostCSS 가 in-process 라 정상 동작(2026-09-11 확인, 첫 컴파일 ~30s). 같은 PC 의 사용자 브라우저에서도 `localhost:3000` 접속 가능. 로그인이 필요한 화면을 자동 캡처하는 절차(헤드 Chrome `--remote-debugging-port` + 사용자 직접 로그인 + DevTools Protocol)는 `docs/제출-주요화면-10.md` 의 "캡처 방법과 상태" 참조 — 제출용 PDF `docs/코드몽_주요화면_11장.pdf`(표지 + 화면 11쪽)와 원본 `docs/screens/` 가 그 산출물.
...
- **프로젝트형 강의(13~15강) 실행 환경**: Pyodide 는 CDN 동적 로드라 첫 진입 시 수 초 로딩(이후 싱글톤 재사용). CodeMirror(`code-editor.tsx`)는 브라우저 전용이라 ProjectRunner 에서 `dynamic(ssr:false)` 로 로드 — 직접 import 하면 SSR 단계에서 깨짐. 파일 입출력은 실행마다 임시 폴더로 격리(이전 실행의 `result.txt` 등이 남지 않음), 함수 안 `input()` 은 대화형 래퍼의 def→async 변환으로 지원(입문자 코드 범위 — `grader.ts` 행 참조). 새 프로젝트 강의 배포 전 `pnpm db:push` 로 `project_progress` 테이블 생성 필요. (강별 연습 문제 통과 저장도 동일 — `exercise_progress` 테이블이 없으면 배포 전 `pnpm db:push` 필요. 성장 레이더 실측용 `exercise_attempts` 테이블도 동일 — 없으면 배포 전 `pnpm db:push` 필요. 커뮤니티 알림도 동일 — `notifications` 테이블이 없으면 배포 전 `pnpm db:push` 필요. 실력향상 제출 기록도 동일 — `problem_submissions`/`weekly_reports` 테이블이 없으면 배포 전 `pnpm db:push` 필요. **2026-07-16 기준 이 두 테이블은 아직 미생성** — Supabase 프로젝트가 pause 상태라 `pnpm db:push` 가 `tenant/user not found` 로 실패함. 대시보드에서 프로젝트 resume 후 재실행할 것.)
...
**영상 정책**:
- 영상 1편 = 강의 1개 (lesson 1대1 매핑). **Python 기초 = 12강 = 영상 12편, 전편 완성·임베드 완료** (확정 커리큘럼은 메모리 `python_curriculum_12.md` 참조). 13~15강은 영상이 아니라 프로젝트형.
...
- **video-voiceover-audio** — Edge TTS 1순위 (`ko-KR-HyunsuMultilingualNeural`, rate `+10%`), ElevenLabs/OpenAI는 키 있을 때만 fallback. BGM/SFX. 자막은 이 에이전트 범위 아님 — 별도 파이프라인(`scripts/generate_captions.py`)이 생성.
...
- 퀴즈 / 채점 / 오답 분석 화면 — `concept` 외 사이드바 탭(개념응용/문제해결/학습완료/성장피드백/다음단계추천)은 "준비 중" 비활성(클릭 불가 + 흐림 + title 툴팁)으로 명시 처리, 라우트·화면 없음. (실력향상 `/skill` 은 stub 에서 **문제 은행 시험 환경으로 승격됨** — 위 Routes 표 참조. AI 3축 채점(C)·레이더 승급(D)·주간 리포트(E, `/mypage/weekly` 대시보드까지) 모두 구현됨.) 1·2강 평가 문제 데이터 60문항 (Pool A 20 + Pool B 40, 모두 `misconceptionId` / `isomorphGroup` / `pool` 라벨링) 은 `lib/quiz-content.ts` 에 정형화돼 있으나, **추천 매칭 로직 자체는 미구현** (backend-developer 영역 — 후보로 룰 베이스 / LLM 기반 / ML 모델 거론). 화면·채점 로직도 미구현. (별개로 프로젝트형 강의(13~15강)의 코드 실행·채점은 `lib/project/grader.ts` 로 구현됨 — 퀴즈 채점과 무관.)
- Python 영상 — **1~12강 전편 완성·임베드 완료** (Hyunsu voice, WebVTT 자막 on/off 연결됨, lesson detail 페이지 임베드 완료). 12강("디버깅 & AI 활용")도 풀렌더(`public/videos/python-lesson-12.mp4`) + lesson detail 임베드(`pythonLesson12Content`) 완료. 13~15강은 영상이 아니라 프로젝트형(계산기 / 랜덤 복습 퀴즈 / 키오스크).
...
- 학습 진도 — **이수율(1층)은 구현 완료**: 영상 강의(90% 시청 + 완료 버튼 → `LessonProgress.learnCompletedAt`) + 프로젝트 강의(채점 통과 → `ProjectProgress.completedAt`) 가 **함께 집계**됨. 단 영상 강의 강 완료는 학습완료에 더해 **그 강 연습 문제 전부 통과(`ExerciseProgress`)까지 AND**(연습 0개 강은 면제, 프로젝트 강의는 연습 트랙 없어 자동 면제). 홈 카드뿐 아니라 **강의 목록·강의 상세 우측 진행률/통계까지 실데이터(`getCourseLessonStatuses`)로 연결** (비로그인이면 전부 not-started). **이해도(2층: 퀴즈 통과 → `quizPassedAt`/`quizBestScore`)만 여전히 미구현** (퀴즈 화면이 없어서 — 그래서 마이페이지 학습 현황에서도 "평균 이해도" 지표를 아예 빼 두었다). `/mypage/page.tsx`의 성장 레이더 카드(`getSkillRadarDetail`)는 **코드 품질 5축을 "나 vs 전체 평균" 으로** 보여줌. **5축 전부 실측 승급 완료(D단계)** — 결정적 2축은 `ExerciseAttempt`+`ProblemSubmission` 합산, AI 3축은 Gemini 점수 평균, 집계는 문제당 최신 제출 1건 규칙. **데모 폴백 없음(2026-09-04)** — 표본 없는 축은 "측정 전", 표본 0이면 차트 미렌더, 코호트 부족이면 평균 곡선 미렌더. **학습 캘린더(`/mypage/calendar`)도 실데이터 연결 완료** — 날짜별 학습 깊이·문법 테마·오답 복습 해결·연속 학습일·날짜별 리포트(`getLearningCalendar`). **시험 D-Day + 약점 단원 보강 추천도 구현 완료**(`ExamSchedule` 모델). **학습 현황 카드·최근 학습·뱃지도 실데이터화 완료**(`lib/learning/stats-queries.ts` — 뱃지는 저장 모델 없이 기존 진도/제출에서 파생). 화면에 보이는 숫자에 mock/데모값은 남아 있지 않다.
- **AI 코드 진단(개념·효율·해석 3축) + LLM 비용 최적화 — 계획(미구현, 방향만 확정)**: 평가 surface 를 객관식 퀴즈가 아니라 **전체 코드 작성 문제(프로그래머스/백준 스타일)** 로 전환해 제출 코드를 5축으로 채점 → 단원별/종합 스파이더 차트를 제공할 예정. 결정적 2축(구문·로직)은 이미 Pyodide/`ExerciseAttempt` 실측, **AI 3축(개념·효율·해석)은 미리 정의한 루브릭을 저가 모델(Gemini 2.5 Flash-Lite / GPT-5-nano)에 주고 감점 근거와 함께 채점**할 계획(seam=`getSkillRadar` 본문 — 시그니처·차트 무변경). LLM 비용 최적화 방침: ① 반복되는 루브릭을 **컨텍스트(프롬프트) 캐싱**(캐시 토큰 ~90% 할인) ② 출력 경량화(JSON 강제모드·점수/짧은 태그·`max_output_tokens` 상한 — 파이프 포맷은 대량 반복 출력에만) ③ **제출 시에만** 호출(타이핑/실행 중 X) ④ 성장 리포트 등 비실시간 작업은 **Batch API**(50% 할인) ⑤ 무료 유저 하루 N회 제한(비용 상한 + 프리미엄 전환 트리거) ⑥ **응답 캐싱은 코드 채점 경로엔 무효**(제출 코드가 매번 달라 히트 안 됨 — 문제 단위 콘텐츠(개념 설명/힌트)에만 부분 적용). 맞춤 문제 추천은 LLM 아닌 **규칙 기반**(약점 축 → 다음 문제). 수익 모델은 프리미엄(AI 3축 무제한·성장 리포트·맞춤 추천) + B2B(강사 대시보드 = 개별 5축 집계) 투 트랙. **설계 근거·학술 인용(논문 DOI)·모델 가격·비용 계산·수익모델·플랫폼 벤치마킹·캡처 URL 은 `docs/발표-근거-리서치.md` 참조** (교수님 피드백 "설계 근거·신뢰성" 대응용).
