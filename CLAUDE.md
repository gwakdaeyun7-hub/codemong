...
- **Prisma 7.8** → Supabase Postgres (singleton: `lib/prisma.ts`, generated client: `lib/generated/prisma/`). 모델 14개: Post / Comment / PostLike / CommentLike / LessonLike / PostReport / CommentReport / LessonProgress / ProjectProgress / ExerciseProgress / ExerciseAttempt / Notification / ProblemSubmission / WeeklyReport
...
| `/mypage/calendar` | 학습 캘린더 | **실데이터 연결 완료** — 연속 학습일 배너 + 월별 캘린더(**색상=그날 대표 문법 테마 / 진하기=학습 깊이**(연함=영상만·시도만, 짙음=문제·연습 통과) / 우상단 체크 배지=오답 복습 해결) + 날짜 클릭 시 학습 리포트(수강 강의, 문제 수·정답률, 오답 태그). 집계는 `getLearningCalendar` (기존 4개 모델만 사용, 새 테이블 없음). 배지 컬렉션은 여전히 mock(획득 조건 모델 없음). 시험 D-Day는 미구현 |
...
| `components/mypage/` | 마이페이지 (사이드바, 프로필 카드, 학습 통계 카드(mock), 성장 레이더 차트(growth-report-card — `getSkillRadar` 코드 품질 5축·나 vs 전체 평균·결정적 2축(구문 정확도·로직 구현도) 실측 / AI 3축(개념·효율·해석)+전체 평균 데모, skill-radar-chart — **순수 SVG, 의존성 0, 두 곡선·N축 동적**), learning-calendar — **Client**(학습 캘린더: 연속일 배너·월 이동·테마 색/깊이 칸·오답 복습 체크 배지·날짜별 리포트. 집계는 서버에서 끝내고 렌더·선택만 담당, 오늘 날짜(KST)도 서버가 prop 으로 전달), 닉네임/비번 변경 폼, settings-section wrapper) |
