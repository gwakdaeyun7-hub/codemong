...
| `/mypage` | 마이페이지 홈 | 프로필 카드 + 학습 통계(mock) + 성장 레이더(코드 품질 5축 전부 실측 승급 — 문제당 최신 제출 집계, 표본 없는 축만 데모 폴백) + 주간 성장 리포트(열 때 생성) + 최근 학습 |
| `/mypage/settings` | 설정 | 닉네임/비밀번호 변경 + 계정 삭제 안내 |
| `/mypage/comments` | 내가 쓴 댓글 | |
| `/mypage/posts` | 내가 쓴 글 | |
| `/mypage/likes` | 좋아요한 강의/글 | 두 섹션 |
| `/mypage/calendar` | 학습 캘린더 | **실데이터 연결 완료** — 연속 학습일 배너 + 월별 캘린더(**색상=그날 대표 문법 테마 / 진하기=학습 깊이**(연함=영상만·시도만, 짙음=문제·연습 통과) / 우상단 체크 배지=오답 복습 해결) + 날짜 클릭 시 학습 리포트(수강 강의, 문제 수·정답률, 오답 태그). 집계는 `getLearningCalendar` (기존 4개 모델만 사용). + **시험 D-Day 카드**(여러 개 등록/삭제, 가장 가까운 시험 크게, **D-14 이내면 약점 단원 보강 추천 알림**, 시험일은 캘린더 칸에 rose 링·점으로 표시). 배지 컬렉션만 여전히 mock(획득 조건 모델 없음) |
...
| `lib/learning/skill-radar.ts` | 성장 레이더(스파이더) 어댑터 — **D단계 실측 승급 완료**. `PYTHON_SKILL_AXES`(코드 품질 5축) + `getSkillRadarDetail(courseId, userId)` → `{points, meta}` (meta=`SkillRadarMeta`: deterministicLive/aiLive/averageLive — 카피 조건부용) + `getSkillRadar`(기존 시그니처 wrapper) + `getLessonSkillRadar(courseId, lessonId, userId)`(단원별 — 사용자 표본 0이면 null). **결정적 2축 = ExerciseAttempt(연습)+ProblemSubmission(시험) 합산 실집계, AI 3축 = ProblemSubmission 의 Gemini 점수 실집계**. **집계 규칙(사용자 확정): 문제/연습당 최신 제출 1건만** (반복 제출로 평균 부풀리기 차단, AI 축은 문제당 최신 ok 1건, 난이도 가중 미적용). 코호트 평균 = 전 유저 실집계 `unstable_cache` 5분(표본 유저<2 또는 샘플<5 이면 데모 평균). 표본 없는 축은 데모 폴백 |
...
| `lib/skill/weekly-report-queries.ts` | 주간 리포트 순수 로직 — `kstWeekStartDate`(KST 월요일, @db.Date PK 규약)/`kstWeekInstantRange`/`buildWeeklyReportData`(주간 제출 수·통과율 + 현재 레이더 스냅샷 + 전주 리포트 대비 delta + weakestAxis)/`toPayload`. Cron 승급 시 build 재사용 seam |
...
| `components/mypage/` | 마이페이지 (사이드바, 프로필 카드, 학습 통계 카드(mock), 성장 레이더 차트(growth-report-card — `getSkillRadar` 코드 품질 5축·나 vs 전체 평균·결정적 2축(구문 정확도·로직 구현도) 실측 / AI 3축(개념·효율·해석)+전체 평균 데모, skill-radar-chart — **순수 SVG, 의존성 0, 두 곡선·N축 동적**), learning-calendar — **Client**(학습 캘린더: 연속일 배너·월 이동·테마 색/깊이 칸·오답 복습 체크 배지·날짜별 리포트. 집계는 서버에서 끝내고 렌더·선택만 담당, 오늘 날짜(KST)도 서버가 prop 으로 전달), exam-dday-card — **Client**(시험 D-Day: 등록/삭제 폼·가까운 시험 D-Day·D-14 이내 보강 추천 알림. mutation 알림은 useToast, 폼 검증은 inline), 닉네임/비번 변경 폼, settings-section wrapper) |
...
- 학습 진도 — **이수율(1층)은 구현 완료**: 영상 강의(90% 시청 + 완료 버튼 → `LessonProgress.learnCompletedAt`) + 프로젝트 강의(채점 통과 → `ProjectProgress.completedAt`) 가 **함께 집계**됨. 단 영상 강의 강 완료는 학습완료에 더해 **그 강 연습 문제 전부 통과(`ExerciseProgress`)까지 AND**(연습 0개 강은 면제, 프로젝트 강의는 연습 트랙 없어 자동 면제). 홈 카드뿐 아니라 **강의 목록·강의 상세 우측 진행률/통계까지 실데이터(`getCourseLessonStatuses`)로 연결** (비로그인이면 전부 not-started). **이해도(2층: 퀴즈 통과 → `quizPassedAt`/`quizBestScore`)는 퀴즈 화면 구현 후 미구현**. streak/배지 추적 모델도 미구현 (`lib/lesson-plan.ts` 의 badges 는 전부 `acquired: false`, 뱃지 카드도 "준비 중"). `/mypage/page.tsx`의 성장 레이더 카드(`getSkillRadarDetail`)는 **코드 품질 5축을 "나 vs 전체 평균" 으로** 보여줌. **5축 전부 실측 승급 완료(D단계)** — 결정적 2축은 `ExerciseAttempt`+`ProblemSubmission` 합산, AI 3축은 Gemini 점수 평균, 집계는 문제당 최신 제출 1건 규칙. 표본 없는 축·표본 부족 코호트 평균만 데모 폴백(카드 카피가 meta 로 실측 여부 표기). **학습 캘린더(`/mypage/calendar`)도 실데이터 연결 완료** — 날짜별 학습 깊이·문법 테마·오답 복습 해결·연속 학습일·날짜별 리포트(`getLearningCalendar`). **시험 D-Day + 약점 단원 보강 추천도 구현 완료**(`ExamSchedule` 모델). 나머지 학습 통계 카드(`MasteryStatsCard`)와 배지 컬렉션만 여전히 mock(배지 획득 조건 모델 없음).
