...
| `lib/course-detail.ts` | `CourseDetail` 타입 + `pythonCourseDetail` + `getCourseDetail(id)`. `stats.totalHours` = **"약 6시간"**(lesson-plan durationMinutes 합 375분 기준 = 영상 12강 265 + 프로젝트 13~15강 40/50/60 — 실력향상 34문제는 별도 트랙이라 제외), `enrolledCount`/`rating`/`reviews` 는 실데이터가 없어 **미노출**(조건부 렌더 — 가짜 수치·후기 금지 원칙) |
