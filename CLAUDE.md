...
| `lib/skill/weekly-report-queries.ts` | 주간 리포트 순수 로직 — `kstWeekStartDate`(KST 월요일, @db.Date PK 규약)/`kstWeekInstantRange`/`buildWeeklyReportData`(주간 제출 수·통과율 + 현재 레이더 스냅샷 + 전주 리포트 대비 delta + weakestAxis)/`toPayload`. **축 스냅샷은 실제 측정된 축만 기록**(미측정 축을 0점으로 넣으면 그 축이 항상 최저가 되어 `weakestAxis` 추천이 왜곡됨). Cron 승급 시 build 재사용 seam |
| `lib/skill/weekly-report-actions.ts` | Server Action: `getOrCreateWeeklyReportAction()` — **"열 때 생성"**(cron/Batch 없음 — Batch 는 무료 티어 불가 확인). 지난주 리포트 있으면 반환, 없고 지난주 제출≥1 이면 생성(복합 PK 멱등, 충돌 시 재조회), 제출 0이면 행 없이 null. `llmComment` 는 Gemini best-effort(숫자 요약만 입력, 실패 시 null — 숫자 리포트는 유지) |
...
| `components/mypage/` | 마이페이지 (사이드바, 프로필 카드, **학습 현황 카드(mastery-stats-card — `LearningStats` prop 으로 실측 4지표. 퀴즈 미구현인 "평균 이해도"는 지표에서 제외)**, **최근 학습 카드(recent-activity-card — `RecentLessonItem[]` prop)**, 성장 레이더 차트(growth-report-card — 코드 품질 5축·나 vs 전체 평균, **표본 0이면 차트 대신 안내 카드 + /skill CTA**, 미측정 축은 "측정 전" 카피, 평균 범례는 `averageLive` 일 때만; skill-radar-chart — **순수 SVG, 의존성 0, N축 동적**, 코호트 표본 부족 시 평균 곡선 미렌더), learning-calendar — **Client**(학습 캘린더: 연속일 배너·월 이동·테마 색/깊이 칸·오답 복습 체크 배지·날짜별 리포트. 집계는 서버에서 끝내고 렌더·선택만 담당, 오늘 날짜(KST)도 서버가 prop 으로 전달), exam-dday-card — **Client**(시험 D-Day: 등록/삭제 폼·가까운 시험 D-Day·D-14 이내 보강 추천 알림. mutation 알림은 useToast, 폼 검증은 inline), 닉네임/비번 변경 폼, settings-section wrapper) |
...
- **주간 성장 리포트의 약한 축 선정이 전 축 동점이면 첫 축을 잡는다(알려진 이슈, 미수정)**: `lib/skill/weekly-report-queries.ts` 의 `weakestAxis` 는 단순 최소값이라 5축이 전부 동점(예: 전부 100)이면 첫 축(구문 정확도)이 약점으로 잡히고, LLM 코멘트가 "만점이지만 구문 정확도를 더 보라"는 억지 권고를 함(2026-09-11 실데이터 1건에서 확인). 고칠 때는 전 축 동점/만점이면 `weakestAxis = null` 로 두고 `weekly-report-actions.ts` 의 코멘트 프롬프트에서 "약한 축 없음" 을 처리할 것. 주간 코멘트 프롬프트는 `scripts/ai-eval/` 회귀 스위트에 포함돼 있지 않음.
...
- 퀴즈 / 채점 / 오답 분석 화면 — `concept` 외 사이드바 탭(개념응용/문제해결/학습완료/성장피드백/다음단계추천)은 "준비 중" 비활성(클릭 불가 + 흐림 + title 툴팁)으로 명시 처리, 라우트·화면 없음. (실력향상 `/skill` 은 stub 에서 **문제 은행 시험 환경으로 승격됨** — 위 Routes 표 참조. 다만 AI 3축 채점(C)·레이더 승급(D)·주간 리포트(E)는 아직 미구현 단계.) 1·2강 평가 문제 데이터 60문항 (Pool A 20 + Pool B 40, 모두 `misconceptionId` / `isomorphGroup` / `pool` 라벨링) 은 `lib/quiz-content.ts` 에 정형화돼 있으나, **추천 매칭 로직 자체는 미구현** (backend-developer 영역 — 후보로 룰 베이스 / LLM 기반 / ML 모델 거론). 화면·채점 로직도 미구현. (별개로 프로젝트형 강의(13강)의 코드 실행·채점은 `lib/project/grader.ts` 로 구현됨 — 퀴즈 채점과 무관.)
