...
| `lib/project/grader.ts` | **클라이언트 전용** 코드 실행+채점 엔진. Pyodide(CDN 동적 로드) 로 `exec`, `input()` 모킹 + `print` stdout 캡처. `matchesExpected` 는 `ExpectedOutput` 순차 소비(숫자 허용오차 1e-9 / 텍스트 부분일치). export: `preloadPyodide` / `runPythonProgram` / `runPythonInteractive`(대화형 실행 — input() 마다 화면 내 입력 모달, 채점 아닌 '직접 돌려보기' 용) / `matchesExpected` / `gradeStep` + 타입 `CaseResult` / `RunResult`. `gradeStep`/`runPythonProgram` 의 `seed?` 옵션은 wrapper 에 `random.seed(N)` 을 주입해 random 을 결정적으로 채점(10강 연습용) |
...
| `lib/learning/lesson-themes.ts` | 학습 캘린더 문법 테마 — `LESSON_THEMES` 4그룹(basics 1~4강 초록 / control 5·6강 보라 / data 7·8강 파랑 / advanced 9~13강 주황) + `themeOfLesson`/`getTheme`. Tailwind 클래스는 동적 생성 불가라 lightClass·deepClass·dotClass 를 정적 문자열로 보유. prisma 미import — 서버·클라 공용 |
...
- Python 영상 — **1~12강 전편 완성·임베드 완료** (Hyunsu voice, WebVTT 자막 on/off 연결됨, lesson detail 페이지 임베드 완료). 12강("디버깅 & AI 활용")도 풀렌더(`public/videos/python-lesson-12.mp4`) + lesson detail 임베드(`pythonLesson12Content`) 완료. 13강은 영상이 아니라 프로젝트형(계산기 만들기).
